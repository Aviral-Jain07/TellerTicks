...
import { youtubeVideos } from '../../lib/mockData';
      ...
      {youtubeVideos.map((video) => (
        ...
        >
          ...
          >
            ...
            <div className="absolute bottom-2 right-2 bg-black/80 text-white text-xs px-1.5 py-0.5 rounded metric-mono">
              {video.duration}
            </div>
          </div>
          <div className={`p-3 flex-grow flex flex-col ${theme === 'dark' ? 'bg-zinc-900/50' : 'bg-white'}`}>
            ...
            <p className="text-xs text-zinc-600 dark:text-zinc-400 mb-2">{video.channel}</p>
